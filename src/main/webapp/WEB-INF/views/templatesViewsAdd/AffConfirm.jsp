...
<li><a href="trackIndex">Tracker Equip</a></li>   <li><a href="addPlan">Planifier  Tâches </a></li>   <li><a href="getReportIndex">Reporting</a> </li> </ul>
    ...
    <div class="art-layout-cell layout-item-0" style="width: 100%" >  
    	...
    	<f:form modelAttribute="editForm" action="searchBtn" method="post">
						
			<tr>
				
					<td><f:errors path="mc" cssClass="error">
						</f:errors></td>
						<td><f:hidden path="page"  value="AffAdd" hint="Enter un Mot clef"  />
						<f:hidden   path="destinPage"  required="true" hint="Searche"  value="Affectation"/> 
						<f:hidden   path="idEquip"  value="${idEquip }" hint="Searche"  />
						<input type="submit" value="Select User" class="btn btn-success"  style="width:30%,height:60%"  title="Cliquer Ici Pour selectionner unpersone à qui Affecter"/></td>
					</tr>										

	
	</f:form> 
       				
       				<c:if test="${ not empty userConfirm }">
			...
			<table title="Tableau d'affiche ListEquip Curent"  class="art-article" border="0" cellspacing="0" cellpadding="0">
					...
					<tr><img src="photoEquip?idEquip=${affectDetails.id }"  class="img-circle" /> </tr>
					...
					<tr><td> Entrer le  ID de la personne Pour Affectation  :</td>
					<td><f:input  type="number" path="idUser"  required="true" hint="Searche"  min="1"/></td></tr>					
					<tr><td> Date Fin  :</td><td><f:input path="dateEndAff" type="date" required="required"/></td></tr>
					
					<tr>					
									<td> 
					
					 <f:hidden path="nameEquip" value="${affectDetails.nameEquip }"/>
					<f:hidden path="idEquip" value="${affectDetails.id }"/>
